...
if (typeof window.MusicAPI === 'undefined') {
  ...
  window.MusicAPI.Volume = {
    ...
    getVolume: function () {
      return parseInt(window.MusicAPI.Volume.slider.getAttribute('aria-valuenow'), 10);
    },
    ...
    increaseVolume: function (amount) {
      ...
      for (var i = 0; i < amount; i++) {
        keyboard.sendKey(window.MusicAPI.Volume.slider, Keyboard.KEY_UP);
      }
    },
    ...
    decreaseVolume: function (amount) {
      ...
      for (var i = 0; i < amount; i++) {
        keyboard.sendKey(window.MusicAPI.Volume.slider, Keyboard.KEY_DOWN);
      }
    }
  };
  ...
  window.MusicAPI.Playback = {
    ...
    getPlaybackTime: function () {
      return parseInt(window.slider.getAttribute('aria-valuenow'), 10);
    },
    ...
    setPlaybackTime: function (milliseconds) {
      ...
      mouse.clickAtLocation(window.slider, x, 0);
    },
    ...
    mutations.forEach(function (m) {
      for (var i = 0; i < m.addedNodes.length; i++) {
        ...
        if (name === 'text-wrapper')  {
          ...
          var duration = parseInt(document.querySelector('#player #slider').getAttribute('aria-valuemax'), 10) / 1000;
